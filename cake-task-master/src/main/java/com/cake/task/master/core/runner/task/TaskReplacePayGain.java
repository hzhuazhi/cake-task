...
//    @Scheduled(cron = "1 * * * * ?")
//    @Scheduled(fixedDelay = 5000) // 每秒执行
    public void orderStatusQueryHuiChao() throws Exception{
//        log.info("----------------------------------TaskReplacePayGain.orderStatusQueryHuiChao()----start");
...
//                }
//
////                log.info("----------------------------------TaskReplacePayGain.orderStatusQueryHuiChao()----end");
//            }catch (Exception e){
//                log.error(String.format("this TaskReplacePayGain.orderStatusQueryHuiChao() is error , the dataId=%s !", data.getId()));
//                e.printStackTrace();
//                // 更新此次task的状态：更新成失败：因为ERROR
//                replacePayGainUpdate = TaskMethod.assembleReplacePayGainUpdate(data.getId(), 2,0, null, null, "异常失败try!");
//                ComponentUtil.taskReplacePayGainService.updateStatus(replacePayGainUpdate);
