...
//                                    }
                                    ...
                                    // #临时代码：分配给固定的“海枣卡商” -end



                                    // 根据卡商与渠道的关联关系查询要分配的卡商
                                    long merchantId = 0;
                                    MerchantChannelModel merchantChannelQuery = TaskMethod.assembleMerchantChannelQuery(0,0,data.getChannelId(), 1, 1);
                                    MerchantChannelModel merchantChannelModel = (MerchantChannelModel)ComponentUtil.merchantChannelService.findByObject(merchantChannelQuery);
                                    if (merchantChannelModel == null || merchantChannelModel.getId() == null){
                                        flag = false;
                                    }else {
                                        MerchantModel merchantModel = (MerchantModel)ComponentUtil.merchantService.findById(merchantChannelModel.getMerchantId());
                                        if (merchantModel == null || merchantModel.getId() == null){
                                            flag = false;
                                        }else{
